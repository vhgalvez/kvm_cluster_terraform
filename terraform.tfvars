# terraform.tfvars
base_image = "/var/lib/libvirt/images/flatcar_image/flatcar_image/flatcar_production_qemu_image.img"
vm_definitions = {
  "bootstrap1"     = { cpus = 1, memory = 1024 },
  "master1"        = { cpus = 2, memory = 2048 },
  "master2"        = { cpus = 2, memory = 2048 },
  "master3"        = { cpus = 2, memory = 2048 },
  "worker1"        = { cpus = 2, memory = 2048 },
  "worker2"        = { cpus = 2, memory = 2048 },
  "worker3"        = { cpus = 2, memory = 2048 },
  "freeipa1"       = { cpus = 1, memory = 1024 },
  "load_balancer1" = { cpus = 1, memory = 1024 },
  "nfs1"           = { cpus = 1, memory = 1024 },
  "postgresql1"    = { cpus = 1, memory = 1024 },
  "bastion1"       = { cpus = 1, memory = 1024 },
  "elasticsearch1" = { cpus = 2, memory = 2048 },
  "kibana1"        = { cpus = 1, memory = 1024 }
}
ssh_keys       = ["ssh-rsa  vhgalvez@gmail.com"]
cluster_name   = "flatcar"
cluster_domain = "example.com"